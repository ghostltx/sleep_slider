...
- 自动记录上次位置、缩放档位和锁定状态；下次启动会恢复上次设置。
- 配置按电脑和 Windows 用户分别保存，不会把一台电脑的位置带到另一台电脑。
- 单实例保护，重复启动不会创建多个浮层。
...
如果没有历史配置，程序首次启动会根据本机任务栏位置自动放置，并默认使用 `0.5倍` 和锁定状态；之后的缩放档位、位置和锁定状态会随本机设置一起记忆。

位置和锁定状态保存在当前 Windows 用户的 `%LOCALAPPDATA%\SleepSlider\settings.json`。换到另一台电脑首次运行时，程序会自动创建该文件。
